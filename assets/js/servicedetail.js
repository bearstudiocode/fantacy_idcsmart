  ...
  if (!_this.data('submit')) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        _this.html('配置升降级');
        console.log(data.indexOf('form-group row configureproduct'))
        ...
        let tmpFunc = function (type, code) {
          $.ajax({
            ...
            success: function (res) {
              ...
              if (res.status == 200) {
                //toastr.success(res.msg)
                window.open(setting_web_url + "/dcim/novnc?password=" + res.data.password + "&url=" + res.data.url, '_blank', 'width=1280,height=680,menubar=no,toolbar=no');
                refreashPowerStatusCycle(id)
              } else {
                toastr.error(res.msg)
              }
