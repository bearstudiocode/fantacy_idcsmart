<script src="{$Setting.web_url}/themes/clientarea/default/assets/libs/echarts/echarts.min.js?v={$Ver}"></script>
    ...
    <div class="row">
        ...
        <div class="d-flex justify-content-between bg-light height_edu">
          <div class="px-3 py-3" style="width: 100%;">
            <div class="float-left pt-2">
              ...
              <div class="mt-2">
                {if $Credit.this_month_bill.subtotal ==''}
                -
                {else}
                <span class="fontws font-size-14">
                {$Credit.prefix}{$Credit.this_month_bill.subtotal}{$Credit.suffix}
                </span>
      ...
      <div class="col-lg-4 col-md-6 marg_il">
        <div class="d-flex justify-content-between bg-light height_edu">
          <div class="px-3 py-3" style="width: 100%;">
            <div class="float-left pt-2">
              <div class="fontws">{$Lang.amount_settled}</div>
				...
				<table class="table tablelist">
					...
					<tbody>
						...
						<tr>
							...
							<td>{$item.payment}</td>
              {if $item.status == 'Paid'}	<td>已支付</td>{/if}
              {if $item.status == 'Unpaid'}	<td>未支付</td>{/if}
              {if $item.status == 'Draft'}	<td>已草稿</td>{/if}
              {if $item.status == 'Overdue'}	<td>已逾期</td>{/if}
              {if $item.status == 'Cancelled'}	<td>被取消</td>{/if}
              {if $item.status == 'Refunded'}	<td>已退款</td>{/if}
              {if $item.status == 'Collections'}	<td>已收藏</td>{/if}
							<td>
