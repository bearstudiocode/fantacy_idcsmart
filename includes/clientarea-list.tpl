...
<div class="h100p user-center_resources table-responsive">
  <table class="table tablelist mb-0 mt-3">
    ...
    <thead class="bg-light">
      <tr>
        <th class="pointer" prop="domainstatus">{$Lang.machine_status}
          <span class="text-black-50 d-inline-flex flex-column justify-content-center ml-1 offset-3">
            <i class="bx bx-caret-up"></i>
            <i class="bx bx-caret-down"></i>
          </span>
        </th>
        <th class="pointer" prop="productname">{$Lang.host_name}
          <span class="text-black-50 d-inline-flex flex-column justify-content-center ml-1 offset-3">
            <i class="bx bx-caret-up"></i>
            <i class="bx bx-caret-down"></i>
          </span>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>{if $list.billingcycle!="free"}{$list.nextduedate|date="Y-m-d H:i"}{/if}</td>
...
<script>
  ...
  function getSourceList(searchType, orderby) {
    ...
    $.ajax({
      type: "get",
      url: '{$Setting.web_url}' + '/clientarea',
      data: searchObj,
