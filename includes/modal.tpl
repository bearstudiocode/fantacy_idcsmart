					...
					<div class="form-group row mb-4">
						...
						<div class="col-sm-8">
							<select class="form-control" class="second_type" name="type" id="secondVerifyType">
								...
								<option value="email" selected>邮件: {$Userinfo.user.email}</option>
								{/if}
								{if $Userinfo.user.phonenumber}
								<option value="phone">短信:
									{$Userinfo.user.phonenumber|substr=0,3}****{$Userinfo.user.phonenumber|substr=7,11}
								</option>
								{/if}
							</select>
						</div>
...
<script src="{$Setting.web_url}/themes/clientarea/default/assets/js/modal.js?v={$Ver}"></script>
