            ...
            {if $Pay.use_credit_limit}
                <!-- <div class="btn-group">
                    ...
                    {if $Pay.action == 'viewbilling'}
                        <a href="{$Setting.web_url}/viewbilling?id={$Pay.invoiceid}&use_credit_limit=0" class="btn btn-default active" role="button">现金支付</a>
                    {else}
                ...
                <div class="pay-area">
                    ...
                    <div class="par_balance">
                        <img src="{$Setting.web_url}/themes/clientarea/default/assets/images/xinyong-bg.png" alt="">
                        <span class="balance_money" style="color:#1676FE"> ￥{$Pay.total}元</span>
                    </div>
                    {/if}
                    <!-- 支付成功遮罩层 -->
                    <div class="pay_area_mask" >
                        <img src="{$Setting.web_url}/themes/clientarea/default/assets/images/paySuccess.svg" alt="" width="100px">
                    </div>
                </div>
            {else}
                <div class="pay-area">
                    ...
                    {if $Pay.PayStatus!='Paid'}
                        {if $Pay.use_credit==1&&$Pay.credit_enough==1}
                            <div class="par_balance">
                                <img src="{$Setting.web_url}/themes/clientarea/default/assets/images/xianjin-bg.png" alt="">
                                <span class="balance_money"> ￥{$Pay.total}元</span>
                            </div>
                        {/if}
                    {/if}
        ...
        <div class="modal-footer mt-2 d-flex justify-content-between ">
            <div class="d-flex align-items-center footer-pay-balance ">
                ...
                <span class="font-weight-bold mr-2">
                   可用信用额 ￥{$Pay.credit_limit_balance}
                </span>
            ...
            <div class="d-flex align-items-center footer-pay-balance ">
                ...
                {if $Action!='recharge'}
                    ...
                    当前余额￥{$Pay.credit}
                        ...
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" name="use_credit" class="custom-control-input use_credit" id="customCheck1" {if $Pay.use_credit==1}checked{/if}>
                            <label class="custom-control-label" for="customCheck1">使用余额支付</label>
                        </div>
                    {/if}
                {/if}
...
<script type="text/javascript" src="{$Setting.web_url}/themes/clientarea/default/assets/libs/qrcode/qrcode.min.js?v={$Ver}"></script>
...
<script>
    ...
    $(function(){
        ...
        if($(".modal-header .paySelect").length == 0 && payStatus!='Paid'){
            ...
            if(is_open_credit_limit!=''&&parseFloat(credit_limit_balance)>=parseFloat(subtotal)){
        ...
        if(patStatus=='Paid'){
            $('.pay_area_mask').css('display','flex');
        ...
        $('#pay .select-pay').on('change',function(){
            ...
            var url = '{$Setting.web_url}' + '/pay?action=recharge';
            $.ajax({
                type: "POST",
        ...
        //获取信息
        function getData(){
            var url = '{$Setting.web_url}' + '/pay?action=billing';
    ...
    function payNow(){
        var url = '{$Setting.web_url}' + '/pay?action=billing&pay=true';
        // var use_credit = $('input[name=use_credit]').prop("checked")
