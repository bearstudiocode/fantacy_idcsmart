...
<script>
  ...
  function payTypeChange(invoiceid) {
    ...
    $.ajax({
        type: "POST",
        data: { invoiceid: invoiceid,paymt :paymt},
        url: url,
        success: function (data) {
          if($('.paySelect option:selected').val() == 0) {
            payamount(invoiceid,0)
          } else {
            payamount(invoiceid)
          }
        }
...
<div id="pay">
  <div class="modal fade pay" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
