...
<link rel="stylesheet" href="{$Setting.web_url}/themes/clientarea/default/assets/libs/ZdCascader/ZdCascader.css?v={$Ver}">
<script type="text/javascript" src="{$Setting.web_url}/themes/clientarea/default/assets/libs/ZdCascader/ZdCascader.js?v={$Ver}"></script>
				...
				<table class="table tablelist">
					...
					<tbody>
						...
						<tr>
							...
							<td>{$list.post}</td>
							<td>
								<a href="#" class="text-{$list.default == '1'?'muted not-allowed':'warning'}" onclick="defaultShippingAddress({$list.id},'{$list.default}')"><i
...
<script>
	function deleteShippingAddress(id) {
		$.ajax({
			type: "DELETE",
			url: '{$Setting.web_url}' + '/voucher/voucherpost',
			data: {
				id
			},
	...
	function defaultShippingAddress(id,isDefault) {
		...
		$.ajax({
			type: "post",
			url: '{$Setting.web_url}' + '/voucher/voucherdefaultpost',
			data: obj,
			success: function (data) {
				if (data.status !== 200) {
					toastr.error(data.msg)
