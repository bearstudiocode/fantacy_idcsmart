...
<div class="card">
	<div class="card-body">
		...
		<div>{$Lang.deduction_invoice}：{$Currency.prefix}{$Invoicedetail.voucher.amount} +
	...
	<div class="card-body">
		<form method="post" class="needs-validation" novalidate>
			...
			<div class="form-group row mb-4">
				...
				<div class="col-sm-5">
					<select class="form-control select2 select2-hidden-accessible" id="courierOptions" data-select2-id="1"
						...
						<option value="{$list.price}" data-courierid="{$list.id}">{$list.name}({$Lang.postage}{$list.price})</option>
						{/foreach}
					</select>
				</div>
			</div>
			<div class="form-group row mb-4">
				...
				<div class="col-sm-11">
					<select class="form-control select2 select2-hidden-accessible selectpicker" id="mailingAddress" data-select2-id="1"
						tabindex="-1" aria-hidden="true">
						{foreach $Issuevoucher.post as $list}
						<option value="CA" selected="{if $list.default == '1'}true{else}false{/if}" data-addressid="{$list.id}" data-content="{if $list.default == '1'}<i class='fas fa-star text-warning mr-2'></i>{else}<i class='fas fa-star text-warning mr-2' style='opacity: 0;'></i>{/if}{$list.province}{$list.city}{$list.region}">
...
<script>
	...
	function confirmPayment() {
		...
		$.ajax({
			type: "post",
			url: '{$Setting.web_url}' + '/voucher/issuevoucher',
	...
	<div class="card-body">
		<div class="table-container">
			...
			<div class="table-responsive">
				...
				<button class="btn btn-primary btn-sm" id="issueBtn">开具发票</button>
			</div>
			<!-- 表单底部调用开始 -->
...
<div class="card">
	<div class="card-body">
		<a href="invoicelist?action=invoiceapply">
			<button type="button" class="btn btn-success btn-sm waves-effect waves-light mb-2">发票申请</button>
		</a>
		<div class="table-container">
				...
				<table class="table tablelist">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								{if $list.amount}
								{$Currency.prefix}{$list.amount}{$Currency.suffix}
								{else}-
								{/if}
							</td>
							...
							<td>
								...
								<a href="javascript: payamount({$list.invoice_id});" class="text-primary mr-2"><i class="fas fa-check-circle"></i> {$Lang.payment}</a>
								{/if}
							</td>
						</tr>
						{/foreach}
