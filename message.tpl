		...
		<div class="table-container">
			...
			<div class="row mt-2">
				<div class="col-12">
					...
					<button type="button" class="btn btn-primary waves-effect waves-light" onclick="allRead()">{$Lang.all_read}</button>
					<button type="button" class="btn btn-danger waves-effect waves-light" onclick="allDel()">{$Lang.delete_all}</button>
				</div>
			</div>
		</div>
	</div>
	...
	$(function () {
		...
		$('.row-checkbox').on('change', function () {
			...
			if ($('.row-checkbox:checked').length) {
				$('#delBtn,#readBtn').removeAttr('disabled').removeClass('not-allowed');
			} else {
				$('#delBtn,#readBtn').attr('disabled', 'disabled').addClass('not-allowed');
			}
		});
	});
</script>
<script>
	...
	function delMsg() {
		...
		$.ajax({
			type: "get",
			url: '{$Setting.web_url}' + '/delete_messgage',
			data: {
				ids
			},
	...
	function readMsg() {
		...
		$.ajax({
			type: "get",
			url: '{$Setting.web_url}' + '/read_messgage',
			data: {
				ids
			},
			success: function (data) {
