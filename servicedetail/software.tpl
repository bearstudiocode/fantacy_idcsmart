...
<script src="{$Setting.web_url}/themes/clientarea/fantacy/assets/libs/moment/moment.js?v={$Ver}"></script>
<style>
  .btnzhuanyi a{
    color: #fff;
        ...
        <div class="row">
          <div class="col-12 col-sm-8 text-white">
            <div class="card card_body bg-primary p-4">
              <div class="mb-4">
                <span
                  class="status-{$Detail.host_data.domainstatus|strtolower} px-2 py-0 rounded-sm fs-12">{$Detail.host_data.domainstatus_desc}</span>
              </div>
          ...
          <div class="col-12 text-black-50">
            <div class="card card_body bg-light p-4">
              <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <button id="resetBtn" onclick="resetAuth('{$Think.get.id}')" class="btn btn-info px-3 py-1 rounded-sm" {if
                  $Detail.module_client_main_area[0]['value']=='' } disabled {/if}>{$Lang.reset_authorization}</button>
                {/if}
              </div>
              <div class="d-flex justify-content-between align-items-center mb-3">
                <div>
                ...
                <span class="bg-info px-3 py-1 rounded-sm text-white">下载文件</span>
                {/if} -->
              </div>
            </div>
          </div>
      ...
      <div class="card card_body p-3">
        <div class="card card_body bg-light fs-12">
          ...
          <div class="mt-4 ml-4">
            ...
            <span
              class="px-2 rounded-sm fs-12 text-white {$Detail.host_data.format_nextduedate.class}">{$Detail.host_data.format_nextduedate.msg}</span>
            {/if}
          </div>
          {/if}
          <div class="my-2 ml-4">
        ...
        <div>
          ...
          <span class="font-weight-bold text-dark fs-14">{$Renew.currency.prefix}{$Renew.host.firstpaymentamount}</span>
        </div>
        <div class="mb-2">
        ...
        <div>
          {if $Renew.host.status == 'Unpaid'}
          <span class="bg-primary px-3 py-1 pointer rounded-sm text-white">{$Lang.pay_immediately}</span>
          {/if}
          {if $Renew.host.status == 'Paid'}
              ...
              <div class="table-header mb-2">
                ...
                <div class="table-search">
                  <div class="row justify-content-end">
                    <div class="col-sm-6">
                      <!-- <input type="text" class="form-control form-control-sm" id="logsearchInp" placeholder="按关键字搜索"> -->
                    </div>
                  </div>
                </div>
              </div>
